...
def _assert_bundle_contents(*, names: set[str]) -> None:
    required_names = {
        ...
        f"{BUNDLE_ROOT}/scripts/check_local_reader_bundle.py",
        f"{BUNDLE_ROOT}/scripts/check_windows_service_task.py",
        f"{BUNDLE_ROOT}/scripts/windows/run_service.ps1",
        f"{BUNDLE_ROOT}/scripts/windows/run_service.cmd",
        f"{BUNDLE_ROOT}/scripts/windows/run_scheduled_service.ps1",
        README_ARCNAME,
        EXTENSION_PACKAGE_ARCNAME,
    }
...
def _run_setup_local(*, bundle_root: Path, python_executable: str) -> dict[str, object]:
    ...
    python_paths = [
        ...
        str(bundle_root / "packages" / "document_import" / "src"),
        str(bundle_root / "packages" / "speech_rules" / "src"),
    ]
...
def _assert_setup_payload(*, setup_payload: dict[str, object], bundle_root: Path) -> None:
    ...
    if not single_installable_model_id and len(
        ...
        raise WindowsBundleBootstrapError(
            "setup-local default catalog did not expose installable choices"
        )
