...
def test_package_windows_bundle_builds_source_and_extension_bundle(
    ...
    assert "tts-platform/scripts/check_extension_library_flow.py" in names
    ...
    assert ".\\.venv\\Scripts\\python.exe -m pip install sherpa-onnx" not in readme
    assert ".\\.venv\\Scripts\\python.exe -m pip install numpy" not in readme
